package com.wiselane.revealcricketquiz;
...
public class DataBaseHelper extends SQLiteOpenHelper {
	...
	private static String DB_NAME = "cricket.sqlite";
	...
	private static final String KEY_BLOCK_COUNTER = "UnlockCounter";
	...
	public int updateBlockInfo(BlockInfo info, String tableName) {
		...
		values.put(KEY_BLOCK_COUNTER, info.getBlockCounter());
	...
	public void resetDatabase() {
		/*String tables[] = new String[] { "BlockOne", "BlockTwo", "BlockThree",
				"BlockFour", "BlockFive", "BlockSix", "BlockSeven" };

		int length = tables.length;

		for (int i = 0; i < length; i++) {
			String tableName = tables[i];
			List<Player> allPlayersFromTable = getAllPlayers(tableName);

			for (int j = 0; j < allPlayersFromTable.size(); j++) {
				Player player = allPlayersFromTable.get(j);
				player.setScore(1000);
				player.setUnlocked(0);
				player.setVisibleParts(String.valueOf(generateRandomNumber()));
				updatePlayer(player, tableName);
			}
		}

		List<BlockInfo> allBlocksInfo = getAllBlocksInfo("BlockInfo");
		int size = allBlocksInfo.size();

		for (int c = 0; c < size; c++) {
			BlockInfo info = allBlocksInfo.get(c);
			info.setBlockProgress(0);
			info.setTotalScore(0);
			info.setUnlockedPlayers(0);
			updateBlockInfo(info, "BlockInfo");
		}*/
